...
class HealthStoreManager: NSObject, ASAuthorizationControllerDelegate, ObservableObject {
    ...
    @objc func appDidBecomeActive() {
        print("App has come to the foreground, refreshing AWS credentials.")
        if let tokenString = UserDefaults.standard.string(forKey: "appleSignInToken") {
            // Just refresh the AWS configuration without asking for Apple Sign-In
            reinitializeAWSConfiguration()
        } else {
    ...
    func isAppleTokenExpiringSoon(tokenString: String) -> Bool {
        // No longer used to trigger re-authentication
        return false // Always return false to avoid prompting sign-in
    }
    ...
    func reinitializeAWSConfiguration() {
        // Use the unauthenticated identity pool configuration
        let credentialsProvider = AWSCognitoCredentialsProvider(
            regionType: .APSoutheast2,
            identityPoolId: "ap-southeast-2:336ac768-2808-4b3a-908f-7d70f9b83dc7" // Unauthenticated identity pool ID
        )

        print("Clearing AWS credentials keychain.")
        credentialsProvider.clearKeychain()

        // Configure AWS services with the unauthenticated provider
